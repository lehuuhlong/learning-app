  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(t("admin.toastSuccess", { count: data.count }));
      } else {
        throw new Error(data.error || t("admin.toastError"));
      }
    } catch (err: any) {
      console.error(err);
      toast.error(err.message || t("admin.apiError"));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex h-[calc(100vh-4rem)] items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t("admin.safeguard")}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="glass-card border-none">
            <CardHeader>
              ...
              <CardDescription>{t("admin.configDesc")}</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleGenerate} className="space-y-6">
                {/* 1. Content Type Selector */}
                <div className="space-y-2">
                  <Label className="text-sm font-semibold">{t("admin.contentType")}</Label>
                ...
                <div className="space-y-2">
                  <Label className="text-sm font-semibold">{t("admin.jlptLevel")}</Label>
            ...
            <CardHeader className="pb-3 border-b border-border/10">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg font-semibold flex items-center gap-2">
                  ...
                  {t("admin.resultsTitle", { count: generatedItems.length })}
                </CardTitle>
              ...
              {loading ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-3 py-16">
                  ...
                  <div className="space-y-1">
                    <p className="font-bold text-foreground text-sm">{t("admin.generatingTitle")}</p>
                    <p className="text-xs text-muted-foreground max-w-xs">
                      {t("admin.generatingDesc")}
                    </p>
                  </div>
                </div>
              ) : generatedItems.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-3 py-16 text-muted-foreground">
                  ...
                  <p className="font-semibold text-sm">{t("admin.emptyTitle")}</p>
                        ...
                        <div className="space-y-2.5">
                          <div className="flex items-baseline justify-between border-b border-border/10 pb-2">
                            ...
                            <Badge variant="secondary" className="text-[10px] bg-violet-500/10 text-violet-500 border border-violet-500/20 font-bold">
                              {t("admin.formation", { formation: item.formation })}
                            </Badge>
                          </div>
                          
                          <div className="text-sm font-bold text-foreground">
                            {t("admin.meaning")}: <span className="text-primary font-medium">{item.meaning}</span>
                          </div>

                          {/* Examples */}
                          <div className="space-y-2 pt-1.5">
                            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{t("admin.examples")}:</span>
